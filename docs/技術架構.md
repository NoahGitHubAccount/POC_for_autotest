...
flowchart TB
    ...
    subgraph L1["第 1 層：輸入"]
        PDF["需求 PDF<br/>高雄數位市民活動模組需求.pdf"]:::input
        CFG["config/config.local.yaml<br/>帳密 + base_url"]:::input
        WBS["WBS.md<br/>工項階層"]:::input
    end
...
實際操作時不會把所有層都動到。下圖是「**一個工項從 0 到 docx 交付**」的單次循環，對應 `docs/合作SOP_QA.md` Q3 的 5 步流程。
...
flowchart LR
    ...
    PDF[/"需求 PDF"/]:::artifact
    WBSPICK["人類挑工項<br/>WBS.md → 給 ID"]:::human
...
| 步驟 | 觸發者 | 讀什麼 | 寫什麼 |
|---|---|---|---|
| spec 展開 | AI | PDF + WBS.md + `prompts/10_*` | `specs/<id>.md` |
...
假設你拿到的是另一個受測網站（不是 dev-maas），只要走以下七步：
...
7. **進入 `docs/合作SOP_QA.md` Q3 的 5 步流程**：spec review → 寫 test → 跑 pytest → 紅了除錯 → docx 交付。
...
| ✅ 保留 | `docs/`（本檔 + 工具指令集 + 合作SOP_QA） | 流程文件，可微調但概念通用 |
| 🔄 替換 | `WBS.md` | 新站的工項階層 |
| 🔄 替換 | 需求 PDF | 新站的規格 |
