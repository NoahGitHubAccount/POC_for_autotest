...
| IT-04 | 圖片尺寸/大小（裁切 + >5MB 錯誤） | 低-中 | 定版 3 案＋待審 1 案 | `specs/integration/IT-04 圖片尺寸與大小防呆.md` | `tests/integration/test_it04_圖片尺寸與大小防呆.py` | 定版 ac1/2/4（3P）；review ac3（XF） | 尺寸不符可傳✓、預覽裁切保留上方✓；**>5MB 未擋=ISS-006**；場次子表單 Banner 待展開 |
| IT-05 | 時間規則防呆（逐條反向） | 低 | 匯集待審 3 案 | `specs/integration/IT-05 時間規則防呆.md` | `tests/integration/test_it05_時間規則防呆.py` | review 3 案（3XF） | 重疊 14 條引用 2-3-13；docx 獨有 3 條全 xfail；**規格矛盾 4 點=ISS-005 待承辦人釐清** |
...
| IT-11 | 報名填寫防呆 | 中（B6 已解） | 匯集待審 4 案 | `specs/integration/IT-11 前台報名填寫防呆.md` | `tests/integration/test_it11_前台報名填寫防呆.py` | review 4 案（2P/2XF） | AC1/AC2 輸入端防呆✓；AC3 格式錯誤可送出 **xfail [ISS-015]**、AC4 必填空值可送出 **xfail [ISS-016]**（B6 造態＝`ops.add_registrant_field`）；分組灰色需後選分組活動 |
