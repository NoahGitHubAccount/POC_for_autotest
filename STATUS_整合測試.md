...
- **2026-07-18 XXVII** — **壓測議題結束收版控＋新階段開跑（業主意見處理）**：
  ...
  - 新階段兩軸：① 業主 Google Sheet 問題單複測（A 欄編號 199~218、222、223 共 21 筆），複測結果收錄報告；② 活動報名頁五類時間（活動/場次/報名/報到/曝光）後台設定 → 前台作動聯動驗證，含「活動啟用 switch × 曝光時間」可見性矩陣（曝光 I/J 規則：不可晚於活動結束時間）。 — **IT-06 AC7 重測（ISS-010 開發回報無法重現→複驗）＋壓測收納活動建立**：
