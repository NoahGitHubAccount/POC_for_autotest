...
| RT-業主 | 業主問題單複測（23 編號：199~218＋235~238） | 低-中 | 匯集待審 15 案列（14P/1XF）＋IT-02/04 迴歸 5XF | `specs/integration/RT-業主 業主問題單複測.md` | `test_rt_業主問題單複測.py`＋`…_公民態.py` | review：no199/200/201/202+212/208a/208b/209/210/211/214/215/216/218/235（14P）＋no213（XF＝未修，系統忙碌 modal）；222/223（我方 ISS-004/006）＝IT-02/04 迴歸 XF 維持 | 台帳 `整合測試_業主複測台帳.md`；餘 205（樣態待業主釐清）＋**237 待自動展開**＋人工 6 筆（204/206/207/217/236/238） |

## 決議紀錄（最新在上）

- **2026-07-18 XXXII** — **226/230 迴歸結案＋237 確認存在（+8h 時區根因）**：
  - **ISS-009（#226 Banner 必填提示）／ISS-012（#230 已結束場次可報名）迴歸 XPASS** → 移除兩案 xfail、素面重跑 PASS → **結案**（問題單 A 主表已標結案＋回歸紀錄補行；run `20260718_regress226_230`）。IT-10 ac6 原 strict xfail 之 XPASS 顯示為 failed 係機制訊息，實質通過。
  - **#237 批量場次時間 vs EXCEL＝確認存在＋根因定位**：新案 `test_rt_no237`（IT-06 ac6 全鏈延伸＋唯一場次名逐場比對）實測 EXCEL 10:00→子表單 18:00、11:00→19:00＝**固定 +8 小時，匯入端把台北時間當 UTC 解析**；xfail 收錄（run `20260718_retest237`）。附帶發現：範本自帶示例列會一併匯入成場次。
  - Selector 探勘落 DOM 事實：場次列時間欄 data-field-id＝`startTime_<列後綴>`/`endTime_<列後綴>`（IT-08 的 sessionStartTime 是 FieldEditRules 鍵非 DOM id）；收合列不渲染完整欄位，讀值前需點「場次 N」標頭展開；以名稱欄 id 後綴配對同列欄位。
  - 三份 run 已匯集入待審；`待審彙整報告.md` 重生成。

- **2026-07-18 XXXI** — **業主 sheet 編號更正＋我方問題單 16 筆取得正式編號＋新增 237/238 兩筆入範圍**：
  - **編號更正**（使用者告知）：sheet 原重複編號已改列——原「222-b 曝光不生效」＝**235**（複測 PASS 不變，test 改名 `test_rt_no235_*`、spec/待審台帳同步就地更正）、原「223-b 掃碼報到」＝**236**（人工）；我方回報單維持 222（ISS-004）/223（ISS-006）。歷史決議條目（XXVII~XXX）中之 222-a/b、223-a/b 記號以本條為準，不回改。
  - **我方 16 筆問題單已入業主 sheet＝219~234**：問題單 B 待貼區 16 筆全數移入 A 主表回填 Sheet 編號（ISS-001→219…ISS-018→234，對照見 A 主表）；B 區僅餘 ISS-020（活動時間列格式，尚未貼上）。⚠ 值得注意：**226（ISS-009 Banner必填提示）與 230（ISS-012 已結束場次可報名）業主標「已完成」**→ 待安排迴歸（IT-02 ac4、IT-10 ac6 xfail 重驗）。
  - **新議題入複測範圍**：**237** 批量建立場次時間與 EXCEL 不同（sheet 草列；可自動化＝IT-06 ac4 批量場次全鏈延伸＋時間逐場比對，待展開）、**238** 官網 SSO 登入態未帶入活動平台（跨站真實登入，人工）。複測範圍更正為 199~218＋235~238 共 23 編號。
