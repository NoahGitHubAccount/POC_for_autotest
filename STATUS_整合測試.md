...
**最後更新**：2026-07-18（壓測議題結束收版控；下一階段＝業主問題單複測 21 筆 + 時間規則前台聯動驗證）／前次 2026-07-13（IT-06 AC7 重測：官方模板原樣上傳成功、ISS-010 縮小為內部錯誤外洩；壓測收納活動建立見 `整合測試_壓測活動記錄.md`）／前次 2026-07-11（登入態衝刺：IT-10 完整報名鏈 ac3/ac4、IT-13 票夾 ac1 全 P；**review 16 案**＝IT-02 ac4、IT-03 ac4、IT-04 ac3/5/6、IT-05×3、IT-06 ac7、IT-09×2、IT-10×4、IT-13×1；xfail 缺陷群 ISS-001~010 週一交開發）
...
| IT-06 | 批次上傳範本（下載→回填→上傳） | 中 | 已定版 4 案＋ac7 重測 2 案 | `specs/integration/IT-06 批次上傳範本.md` | `tests/integration/test_it06_批次上傳範本.py` | 定版 ac2/4（全鏈）＋ac5/6（4P，2026-07-11 使用者審查）；ac7 改版重測（2026-07-13：ac7 P／ac7b XF） | 白/黑名單+批量新增區域/場次 全通；**ac7 重測（07-13，開發回報 ISS-010 無法重現後複驗）**：官方模板**原封不動上傳匯入成功**「共新增 1 筆工作人員」（07-11「官方模板必失敗」不復現）→ ac7 改寫為「下載→原樣上傳」；數字 ID 仍外洩 cast Int64 內部錯誤 → 新增 ac7b 對照案 xfail [ISS-010 縮小範圍後仍成立，問題單描述已更新] |
...
| IT-17 | 曝光時間與前台可見性（曝光×啟用矩陣＋顯示格式） | 低 | 匯集待審 4 案 | `specs/integration/IT-17 曝光時間與前台可見性.md` | `tests/integration/test_it17_曝光與時間前台聯動.py` | review ac1/3/4（3P）＋ac5（XF） | 矩陣三格 P（曝光中可見/曝光過不可見/未啟用不可見）；AC2＝RT no222b（P）；ac5 活動時間列格式 xfail=**ISS-020**；待展開「自動改非啟用」語意 |
| RT-業主 | 業主問題單複測（21 編號 23 筆） | 低-中 | 匯集待審 15 案列（14P/1XF）＋IT-02/04 迴歸 5XF | `specs/integration/RT-業主 業主問題單複測.md` | `test_rt_業主問題單複測.py`＋`…_公民態.py` | review：no199/200/201/202+212/208a/208b/209/210/211/214/215/216/218/222b（14P）＋no213（XF＝未修，系統忙碌 modal）；222a/223a=IT-02/04 迴歸 XF 維持 | 台帳 `整合測試_業主複測台帳.md`；**自動化複測完成**；餘 205（樣態待業主釐清）＋人工 5 筆（204/206/207/217/223-b） |

## 決議紀錄（最新在上）

- **2026-07-18 XXX** — **業主複測收尾波（no218/no200，2P）**：
  - **no218 PASS**：src 解碼確立寫入路徑 `PATCH /reventmodule/Basic/EVCredentialSetting {id, credentialNote}`（查 id 用 `?EVEventIds=<pkid>`）→ 後台改備註回讀一致＋前台憑證頁顯示新文字＝修復生效（alpha.57）。
  - **no200 PASS（量測）**：票夾載入 1.9 秒（domcontentloaded 即完成），「載入太久」已改善；正式門檻待業主訂。
  - 已匯集 `20260718_retestB3` 入待審（RT-業主 台帳累計 15 案列）；`待審彙整報告.md` 重生成。
  - **自動化複測到此全數完成**：21 編號中 15 編號自動驗畢（PASS 13：199/200/201/202/208/209/210/211/212/214/215/216/218＋222-b；未修 2：213、222-a/223-a 群）；餘＝205（樣態待業主釐清）、207/204/206/217/223-b（人工）、IT-17「自動改非啟用」語意待釐清。
- **2026-07-18 XXIX** — **業主複測 B 群收工（公民態 8 案：7P/1XF）**：
  - **複測 PASS 8 個編號**（run 20260718_retestB1/B2，檔 `test_rt_業主問題單複測_公民態.py`）：no199 特殊符號emoji被擋、no201 名額以參與人數扣減（2人報名→guest額滿）、no202+212 取消名額釋放+再報成功、no208b 已報名直開報名頁轉憑證（208 全案收攏）、no209 滿意度問卷送出成功、no210 無亂碼、no211 憑證頁資料完整。
  - **no213 未修（XFAIL 收錄）**：名額2報3人被擋，但 modal 仍「報名失敗 系統忙碌中，請再試一次」應為「報名人數已滿」——業主 sheet 本無完成標記，複測確認現況（截圖入報告）。
  - **no207 改人工**：真實 LINE 手機認證無法自動化，請使用者下次手動登入觀察落點。
  - 造態複製品全清（RTB1~5/RT215/RT222B/IT17AC3/4 掃描歸零）；admin token 途中過期一次（OCR warm-login 即刷）；公民登入態 2026-07-18 手機認證更新。
  - 尚餘：218（credentialNote 寫入路徑待探勘）、205（「不合規樣態」待業主釐清）、C 群人工 5 筆。
  - **匯集完成（同日補）**：RT-業主/IT-17 以完整重跑產完整報告（`20260718_RTfull` 13 案 12P/1XF 一次過；`20260718_IT17full` 4 案 3P/1XF）後 collect_to_review 入待審；IT-02 ac5×3、IT-04 ac3 迴歸證據亦入待審（ac6 該次截圖缺、留 07-13 證據列，結論同）。待審總表 `reports/review/待審彙整報告.md` 已重生成。⚠ 教訓：**同 run_id 連跑多次 pytest，報告 md 會被最後一次覆蓋**——要匯集前先完整跑一次。 — **業主複測首日執行（A 群收工）＋IT-17 開跑＋ISS-020 開案**：
  - **複測 PASS 5 筆**（run 20260718_retestA2）：no214（時間地點顯示，業主標待處理但實測已正常）、no215（跨日場次含結束日，含造態跨日驗證）、no216（tab 新標題）、no208a（未登入半段：場次卡全「報名及查詢」＋點擊導 ktc 登入）、no222b（曝光開始未到不可見，v1.6.0-alpha.59 修復確認）。
  - **複測維持 XFAIL 5 案**（run 20260718_retestA1）：222-a＝IT-02 ac5e/f/g（ISS-004 未修）、223-a＝IT-04 ac3/ac6（ISS-006 未修）——與業主「待處理」一致。
  - **IT-17 曝光矩陣**：ac1 曝光中可見／ac3 曝光已過不可見／ac4 未啟用不可見 全 P；ac5 前台「活動時間」列無民國年＋12/24 制混用（場次卡正確）＝**ISS-020 開案**（問題單 B 區已補 TSV）。
  - DOM 事實回填 FrontStage.md：資訊頁「報名及查詢」＝`<button>`（列表頁才是 `<a href*=evEventId>`）；「查看全部」展開 modal 會攔截點擊；SPA 首導 bootstrap 可能先跳 Home（直開需落點重試）。
  - 造態複製品皆 finally DELETE 清理；admin session 已 OCR warm-login 刷新；**公民登入態待使用者手機認證**（信號檔腳本 `tools/_tmp_citizen_manual_login.py`，B 群 10 筆卡此）。 — **壓測議題結束收版控＋新階段開跑（業主意見處理）**：
  - 壓測（2026-07-13~15）產物入版控：`壓測腳本_AppPerfect/`、`壓測腳本_JMeter/`（`shared/token.txt`、`tokens.csv` 為真實 JWT 已 gitignore；`reports/` 子目錄含 run150/run300 zip 依既有規則不入 git、本機保留）、`整合測試_壓測活動記錄.md`；07-15 A 桶批次清理 281 筆（`tools/delete_bucketA.py`，281/281 成功、複查歸零）紀錄已在清理表。壓測議題就此關閉。
  - 新階段兩軸：① 業主 Google Sheet 問題單複測（A 欄編號 199~218、222、223 共 21 編號 23 筆，222/223 各兩筆共用編號；其中 222-a/223-a 為我方回報 ISS），複測結果收錄報告；② 活動報名頁五類時間（活動/場次/報名/報到/曝光）後台設定 → 前台作動聯動驗證，含「活動啟用 switch × 曝光時間」可見性矩陣（曝光 I/J 規則：不可晚於活動結束時間；業主 222-b「曝光開始不生效」已修 v1.6.0-alpha.59 待複測）。
  - 台帳＋分群執行順序＋IT-17 範圍界定：`整合測試_業主複測台帳.md`（sheet 快照 2026-07-18 via published CSV）。 — **IT-06 AC7 重測（ISS-010 開發回報無法重現→複驗）＋壓測收納活動建立**：
  - 探測腳本 `tools/probe_it06_ac7_raw_upload.py`（原封不動上傳官方模板）：模板表頭=（工作人員市民碼ID、備註）、示例列=GUID `e5c62441-…`＋「林O章」；**原樣上傳匯入成功** toast「共新增 1 筆工作人員」、空狀態消失——07-11「照官方模板必失敗」**不復現**，先前失敗係我方 openpyxl 改資料（純數字 ID）所致的面向；對照探測數字 ID 90000099 **仍**回「匯入失敗：Unable to cast object of type 'System.String' to type 'System.Int64'.」＝內部錯誤外洩缺陷仍在。
  - **ISS-010 縮小範圍**（問題單描述已更新）：官方模板可用；缺陷聚焦「非 GUID 資料時外洩後端內部例外，應為清楚格式驗證訊息」。
  - 測試改版：`test_it06_ac7_工作人員名單_下載原樣上傳`（原樣上傳，PASS）＋`test_it06_ac7b_…數字ID上傳_錯誤應非內部例外`（xfail [ISS-010]，單獨跑 XFAIL 確認）。⚠ 同 session 連跑時首案偶發列表頁 15s 逾時 flake、ac7b toast 時序偶漏（strict=False 容忍），單跑皆穩。
  - Session 補充：admin.json mtime 未過期但實際已失效（被導回登入頁）＝mtime 新鮮度判斷有盲點；以臨時 OCR warm-login（`tools/_tmp_warm_login_ocr.py`，ddddocr 一次命中）刷新。
...
| 2026-07-13 | **AppPerfect LoadTest 壓測收納活動×2（平行 session 各建一筆，詳見 `整合測試_壓測活動記錄.md`；建議主用 2027 遠期窗版 34185353624633344）** | 34185353624633344（LoadTest_壓測用_20260713）、34185347837804544（LoadTest_壓測用_registration_open_0713162324） | **保留勿刪**（壓測資料收納） |
| 2026-07-13 | IT-06 AC7 重測：官方模板原樣上傳＋數字ID對照，各在第一列活動新增工作人員 1 筆（GUID 示例 1 筆成功；數字 ID 匯入失敗未寫入） | 第一列活動的工作人員名單 +1（GUID e5c62441-…） | 未清理（影響極小） |
...
**2026-07-15 批次清理**（使用者核准，A 桶）：以「建立者=蘇哲正(sam.su 自動化帳號) + 名稱特徵(`IT0?\d`/`PROBE_`/`_copy`/`LoadTest_`/`AItest_`/`未命名活動`)」判定，共 **281 筆** DELETE、0 失敗；活動總數 330→49，A 桶殘留複查=0。**保留 3 筆**：壓測收納 `34185325523320832`、真實大港閱冰 `34155861376110592`、全開條件活動 `34168368758263808`。**B 桶 28 筆**（蘇哲正建但具名，疑手動測試，含 07-12/07-15 近期）與其他同事帳號資料未動。腳本 `tools/delete_bucketA.py`（雙重保險：非自動化帳號一律不刪；admin session 以 OCR warm-login 刷新）。
